...
const QuantityDayComponent = () => {
    ...
    const { refetch: getQuantityDay } = useGetQuantityDailyQuery();
        ...
        if (
            ...
            getQuantityDay({
                ...
                .then((res) => {
                    ...
                    if (res?.data?.GetQuantityDaily) {
                        const obj = {
                            siteid: currentSiteReport,
                            data: res?.data?.GetQuantityDaily,
                        };
                        temp.push(obj);
                    }
        ...
        if (
            ...
            if (choiceSite[0] === 'Select all') {
                for (const site of siteData) {
                    ...
                    if (temp?.data?.GetQuantityDaily) {
                        const obj = {
                            //@ts-ignore
                            siteid: site.SiteId,
                            data: temp?.data?.GetQuantityDaily,
                        };
                        res.push(obj);
                    }
                }
                    ...
                    <Table.Th colSpan={totalMerger}>
                        <Center>
                            <Text tt="uppercase" fw="bold">
                                Sản Lượng Ngày Của Các Điểm Đo
                            </Text>
                        </Center>
                    </Table.Th>
                </Table.Tr>
                <Table.Tr>
                    <Table.Th colSpan={totalMerger}>
                        <Center>
                            <Text tt="uppercase" fw="bold">
                                Từ ngày{' '}
                                {convertDateToStringNotTimeForTitle(startDate)}{' '}
                                đến ngày{' '}
                                {convertDateToStringNotTimeForTitle(endDate)}{' '}
                                (06: 00)
                            </Text>
                        </Center>
                    </Table.Th>
    ...
    const onExportExcelClicked = () => {
        ...
        a.href = url;
        a.download = `San_Luong_Theo_Ngay_Tu_${convertDateToStringNotTimeForTitle(
            startDate,
        )}_Den_${convertDateToStringNotTimeForTitle(endDate)}.xlsx`;
                        ...
                        {contentTable !== null ? (
                            <Table.Tfoot>
                                <Table.Tr>
                                    <Table.Td colSpan={5}>
                                        <Text c="red" fw={500}>
                                            Ghi chú: Dữ liệu ngày tính từ 6h
                                            sáng hôm trước đến 6h sáng hôm sau.
                                        </Text>{' '}
                                    </Table.Td>
                                </Table.Tr>
                            </Table.Tfoot>
...
export default QuantityDayComponent;
