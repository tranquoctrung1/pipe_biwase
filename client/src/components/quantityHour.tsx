...
const QuantityHourComponent = () => {
    ...
    const { refetch: getQuantityHourly } = useGetQuantityHourlyQuery();
        ...
        if (
            ...
            getQuantityHourly({
                ...
                .then((res) => {
                    ...
                    if (res?.data?.GetQuantityHourly) {
                        const obj = {
                            siteid: currentSiteReport,
                            data: res?.data?.GetQuantityHourly,
                        };
                        temp.push(obj);
                    }
        ...
        if (
            ...
            if (choiceSite[0] === 'Select all') {
                for (const site of siteData) {
                    const temp = await getQuantityHourly({
                        //@ts-ignore
                        siteid: site.SiteId,
                        start: startDate?.getTime().toString(),
                        end: endDate?.getTime().toString(),
                    ...
                    if (temp?.data?.GetQuantityHourly) {
                        const obj = {
                            //@ts-ignore
                            siteid: site.SiteId,
                            data: temp?.data?.GetQuantityHourly,
                        };
                        res.push(obj);
                    }
                }
            ...
            <>
                ...
                <Table.Tr>
                    <Table.Th colSpan={totalMerger}>
                        <Center>
                            <Text tt="uppercase" fw="bold">
                                Từ ngày {convertDateToString(startDate)} đến
                                ngày {convertDateToString(endDate)}
                            </Text>
                        </Center>
                    </Table.Th>
                </Table.Tr>
    ...
    const onExportExcelClicked = () => {
        ...
        a.href = url;
        a.download = `San_Luong_Theo_Gio_Tu_${convertDateToString(
            startDate,
        )}_Den_${convertDateToString(endDate)}.xlsx`;
            ...
            <Grid.Col span={{ base: 12, md: 3 }}>
                ...
                <div>
                    <input
                        ...
                        id="startDate"
                        //@ts-ignore
                        value={convertDateToSetValueDateTimeLocalInput(
                            startDate,
                        )}
                        style={{
                            width: '100%',
            ...
            <Grid.Col span={{ base: 12, md: 3 }}>
                ...
                <div>
                    <input
                        ...
                        id="endDate"
                        //@ts-ignore
                        value={convertDateToSetValueDateTimeLocalInput(endDate)}
                        style={{
                            width: '100%',
                            outline: 'none',
                            padding: '5px',
...
export default QuantityHourComponent;
