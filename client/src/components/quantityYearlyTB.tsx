...
import { useGetQuantityYearlyTb2Query } from '../__generated__/graphql';

import { getDayOfYear, formatNumber } from '../utils/utils';

import * as XLSX from 'xlsx';

const QuantityYearlyTB = () => {
    ...
    const { refetch: getQuanityYearlyTB2 } = useGetQuantityYearlyTb2Query();
    ...
    const onViewClicked = () => {
        ...
        if (isAllowStartDate && isAllowEndDate) {
            ...
            getQuanityYearlyTB2({
                ...
                .then((res) => {
                    if (res?.data?.GetQuantityYearlyTB2) {
                        setLoading(false);
                        renderTable(res.data.GetQuantityYearlyTB2);
                    }
                })
                                ...
                                <Table.Th colSpan={5}>
                                    <Center>
                                        <Text tt="uppercase" fw="bold">
                                            Bảng Tổng Hợp Sản Lượng Sản Xuất Các
                                            Năm Tại Trạm II
                                        </Text>
                                    </Center>
                                </Table.Th>
...
export default QuantityYearlyTB;
